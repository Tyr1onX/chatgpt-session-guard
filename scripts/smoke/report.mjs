...
export function smokeReportMarkdown(report) {
  ...
  if (scroll) {
    lines.push(
      ...
      `- Planned phases: ${Array.isArray(scroll.metrics.plannedPhases) ? scroll.metrics.plannedPhases.join(' → ') : 'n/a'}`,
      `- Executed phases: ${Array.isArray(scroll.metrics.executedPhases) ? scroll.metrics.executedPhases.join(' → ') : 'n/a'}`,
      `- Wheel events: ${value(scroll.metrics.executedWheelEvents)} / ${value(scroll.metrics.plannedWheelEvents)}`,
      `- Transient samples: ${value(scroll.metrics.executedSamples)} / ${value(scroll.metrics.plannedSamples)}`,
      `- Realistic plan complete: ${value(scroll.metrics.realisticPlanComplete)}`,
      `- Selected scroller: ${value(scroll.metrics.selectedScrollerTag)}; scrollHeight=${value(scroll.metrics.selectedScrollerScrollHeight)}; clientHeight=${value(scroll.metrics.selectedScrollerClientHeight)}`,
      `- ScrollTop: initial=${value(scroll.metrics.initialScrollTop)}; first-after-wheel=${value(scroll.metrics.firstAfterWheelScrollTop)}`,
