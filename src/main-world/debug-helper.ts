...
function sanitizeMetrics(value: unknown): DebugMetrics | null {
  ...
  return {
    ...
    renderedMessages: finiteNumber(item.renderedMessages) ?? 0,
    totalMessages: finiteNumber(item.totalMessages) ?? 0,
    configuredHistoryCount: finiteNumber(item.configuredHistoryCount) ?? 0,
    historyUnit: item.historyUnit === 'message' ? 'message' : 'round',
    limitedByDomBudget: item.limitedByDomBudget === true,
